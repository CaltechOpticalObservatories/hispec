...
int bus_address=GAMMA_BUS_ADDRESS;	/* bus address, 1 for RS-232	*/

/*************************************************************************
 *+
 * Function name: gamma_read_version
 ...
 * Outputs: Returns GAMMA error code.
     char *version

 * Modification History:
      2012/01/10 SK  -- Initial.
      2014/02/04 DN adapted to GAMMA controller
 *-
*************************************************************************/
int gamma_read_version(char *port, char *version) {
	...
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_READ_VERSION, NULL, 1)) < 0) 
		return err;

	if ( (err=gamma_send_request(port, command, response)) < 0 )
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response, 
			GAMMA_COMMAND_READ_VERSION)) < 0 )
		return err;
	...
	if ( (err=getStringFromGammaResponse(response, version)) < 0 )
		return err;

  	log_msg(SINFO, LOGLVL_USER1, "%s %d : GAMMA Firmware version: %s", 
	...
	return err;
}	/* end * Function name: gamma_read_version */


/*************************************************************************
 *+
 * Function name: gamma_reset
...
int gamma_reset(char *port) {
	...
	/* create command string */
	if ( (err=gamma_create_command_string(command, bus_address, 
			GAMMA_COMMAND_RESET, NULL, 1)) < 0) 
		return err;

	if ( (err=gamma_send_command(port, command)) < 0 )
		return err;
	...
	return err;
}	/* end * Function name: gamma_reset */


/*************************************************************************
 *+
 * Function name: gamma_set_arc_detect
...
int gamma_set_arc_detect(char *port, int yesno) {
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_SET_ARC_DETECT)) < 0)
		return err;
	...
	return err;
}	/* end * Function name: gamma_set_arc_detect */


/*************************************************************************
 *+
 * Function name: gamma_get_arc_detect
...
int gamma_get_arc_detect(char *port, int *yesno) {
	...
	/* create command string */
	if ( (err=gamma_create_command_string(command, bus_address, 
			GAMMA_COMMAND_GET_ARC_DETECT, NULL, 1)) < 0) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_GET_ARC_DETECT)) < 0 )
		return err;
	...
	/* extract pressure from string */
	if ( (err=getStringFromGammaResponse(response, stryesno)) < 0 )
		return err;
	...
	return err;
}	/* end * Function name: gamma_get_arc_detect */


/*************************************************************************
 *+
 * Function name: gamma_read_current
 ...
 * Modification History:
      ...
      2014/02/04 DN Modified for GAMMA controller
 *-
*************************************************************************/
int gamma_read_current(char *port, float *outcurrent) {
	...
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_READ_CURRENT, NULL, 1)) < 0 ) 
		return err;
	...
	*outcurrent = getFloatFromGammaResponse(response);

	log_msg(SINFO, LOGLVL_USER3, "%s %d: exiting %s", 
			__FILE__, __LINE__, __FUNCTION__);

	return err;
}	/* end * Function name: gamma_read_current */


/*************************************************************************
 *+
 * Function name: gamma_read_pressure
...
int gamma_read_pressure(char *port, float *outpressure) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_READ_PRESSURE, NULL, 1)) < 0 ) 
		return err;
	...
	/* extract pressure from string */
	*outpressure = getFloatFromGammaResponse(response);

	log_msg(SINFO, LOGLVL_USER3, "%s %d: exiting %s", 
			__FILE__, __LINE__, __FUNCTION__);

	return err;
}	/* end * Function name: gamma_read_pressure */


/*************************************************************************
 *+
 * Function name: gamma_read_voltage
...
int gamma_read_voltage(char *port, int *outvoltage) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_READ_VOLTAGE, NULL, 1)) < 0 ) 
		return err;
	...
	*outvoltage = getIntFromGammaResponse(response);

	log_msg(SINFO, LOGLVL_USER3, "%s %d: exiting %s", 
			__FILE__, __LINE__, __FUNCTION__);

	return err;
}	/* end * Function name: gamma_read_voltage */


/*************************************************************************
 *+
 * Function name: gamma_set_units
...
int gamma_set_units(char *port, int units) {
	...
	/* check input units */
	if (units == 'M' || units == 'm') {
		gamma_units[0] = GAMMA_UNITS_MBAR;
	} else if (units == 'P' || units == 'p') {
		gamma_units[0] = GAMMA_UNITS_PASCAL;
	} else	gamma_units[0] = GAMMA_UNITS_TORR;
	gamma_units[1] = '\0';

	/* create command string */
	if ( (err=gamma_create_command_string(command, bus_address, 
			GAMMA_COMMAND_SET_PRESS_UNITS, gamma_units, 1)) < 0) 
		return err;
	...
	return err;
}	/* end * Function name: gamma_set_units */


/*************************************************************************
 *+
 * Function name: gamma_get_pump_size
...
int gamma_get_pump_size(char *port, int *outsize) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_GET_PUMP_SIZE, NULL, 1)) < 0 ) 
		return err;
	...
	*outsize = getIntFromGammaResponse(response);

	log_msg(SINFO, LOGLVL_USER3, "%s %d: exiting %s", 
			__FILE__, __LINE__, __FUNCTION__);

	return err;
}	/* end * Function name: gamma_get_pump_size */


/*************************************************************************
 *+
 * Function name: gamma_set_pump_size
...
int gamma_set_pump_size(char *port, int size) {
	...
	if (size >= 0 && size <= 9999) {
		...
		return GAMMA_ERROR_VALUE_OUT_OF_RANGE;
	}

	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_SET_PUMP_SIZE, strsize, 1)) < 0 ) 
		return err;
	...
	return err;
}	/* end * Function name: gamma_set_pump_size */


/*************************************************************************
 *+
 * Function name: gamma_get_cal_factor
...
int gamma_get_cal_factor(char *port, float *outcalfact) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_GET_CAL_FACTOR, NULL, 1)) < 0 ) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_GET_CAL_FACTOR)) < 0)
		return err;
	...
	*outcalfact = getFloatFromGammaResponse(response);

	log_msg(SINFO, LOGLVL_USER3, "%s %d: exiting %s", 
			__FILE__, __LINE__, __FUNCTION__);

	return err;
}	/* end * Function name: gamma_get_cal_factor */


/*************************************************************************
 *+
 * Function name: gamma_set_cal_factor
...
int gamma_set_cal_factor(char *port, float calfact) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_SET_CAL_FACTOR, strcalfact, 1)) < 0 ) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_SET_CAL_FACTOR)) < 0)
		return err;
	...
	return err;
}	/* end * Function name: gamma_set_cal_factor */


/*************************************************************************
 *+
 * Function name: gamma_set_auto_restart
...
int gamma_set_auto_restart(char *port, int yesno) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_SET_AUTO_RESTART, stryesno, 1)) < 0 ) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_SET_AUTO_RESTART)) < 0)
		return err;
	...
	return err;
}	/* end * Function name: gamma_set_auto_restart */


/*************************************************************************
 *+
 * Function name: gamma_get_auto_restart
...
int gamma_get_auto_restart(char *port, int *yesno) {
	...
	/* create command string */
	if ( (err=gamma_create_command_string(command, bus_address, 
			GAMMA_COMMAND_GET_AUTO_RESTART, NULL, 1)) < 0) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_GET_AUTO_RESTART)) < 0 )
		return err;
	...
	return err;
}	/* end * Function name: gamma_get_auto_restart */


/*************************************************************************
 *+
 * Function name: gamma_pump_start
...
int gamma_pump_start(char *port) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_START_PUMP, NULL, 1)) < 0 ) 
		return err;
	...
	return err;
}	/* end * Function name: gamma_pump_start */


/*************************************************************************
 *+
 * Function name: gamma_pump_stop
...
int gamma_pump_stop(char *port) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_STOP_PUMP, NULL, 1)) < 0 ) 
		return err;
	...
	return err;
}	/* end * Function name: gamma_pump_stop */


/*************************************************************************
 *+
 * Function name: gamma_lock_keypad
...
int gamma_lock_keypad(char *port, int lock) {
	...
	int err=0;
	int command_code = ((lock == 1) ?  GAMMA_COMMAND_LOCK_KEYPAD :
		 			   GAMMA_COMMAND_UNLOCK_KEYPAD);
	char command[MAX_COMMAND_LENGTH];
	char response[MAX_RESPONSE_LENGTH];
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response, command_code)) < 0)
		return err;
	...
	return err;
}	/* end * Function name: gamma_lock_keypad */


/*************************************************************************
 *+
 * Function name: gamma_get_analog_mode
...
int gamma_get_analog_mode(char *port, int *outmode) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_GET_ANALOG_MODE, NULL, 1)) < 0 ) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_GET_ANALOG_MODE)) < 0)
		return err;
	...
	*outmode = getIntFromGammaResponse(response);

	log_msg(SINFO, LOGLVL_USER3, "%s %d: exiting %s", 
			__FILE__, __LINE__, __FUNCTION__);

	return err;
}	/* end * Function name: gamma_get_analog_mode */


/*************************************************************************
 *+
 * Function name: gamma_set_analog_mode
...
int gamma_set_analog_mode(char *port, int mode) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_SET_ANALOG_MODE, strmode, 1)) < 0 ) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_SET_ANALOG_MODE)) < 0)
		return err;
	...
	return err;
}	/* end * Function name: gamma_set_analog_mode */


/*************************************************************************
 *+
 * Function name: gamma_high_voltage_on
...
int gamma_high_voltage_on(char *port, int *yesno) {
	...
	/* create command string */
	if ( (err=gamma_create_command_string(command, bus_address, 
			GAMMA_COMMAND_IS_HIGH_VOLTAGE_ON, NULL, 1)) < 0) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_IS_HIGH_VOLTAGE_ON)) < 0)
		return err;
	...
	return err;
}	/* end * Function name: gamma_high_voltage_on */


/*************************************************************************
 *+
 * Function name: gamma_set_hv_autorecovery
...
int gamma_set_hv_autorecovery(char *port, int mode) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_SET_HV_AUTORECOVERY, strmode, 1)) < 0 ) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_SET_HV_AUTORECOVERY)) < 0)
		return err;
	...
	return err;
}	/* end * Function name: gamma_set_hv_autorecovery */


/*************************************************************************
 *+
 * Function name: gamma_get_hv_autorecovery
...
int gamma_get_hv_autorecovery(char *port, int *outmode) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_GET_HV_AUTORECOVERY, NULL, 1)) < 0 ) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_GET_HV_AUTORECOVERY)) < 0)
		return err;
	...
	*outmode = getIntFromGammaResponse(response);
	...
	return err;
}	/* end * Function name: gamma_get_hv_autorecovery */


/*************************************************************************
 *+
 * Function name: gamma_set_comm_mode
...
int gamma_set_comm_mode(char *port, int mode) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_SET_COMM_MODE, strmode, 1)) < 0 ) 
		return err;
	...
	return err;
}	/* end * Function name: gamma_set_comm_mode */


/*************************************************************************
 *+
 * Function name: gamma_get_comm_mode
...
int gamma_get_comm_mode(char *port, int *outmode) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_GET_COMM_MODE, NULL, 1)) < 0 ) 
		return err;
	...
	*outmode = getIntFromGammaResponse(response);
	...
	return err;
}	/* end * Function name: gamma_get_comm_mode */


/*************************************************************************
 *+
 * Function name: gamma_set_comm_interface
...
int gamma_set_comm_interface(char *port, int interface) {
	...
	/* generate command string */
	if ( (err=gamma_create_command_string(command, bus_address,
			GAMMA_COMMAND_SET_COMM_INTERFACE, strinterface, 1)) < 0 ) 
		return err;
	...
	/* check for errors */
	if ( (err=gamma_validate_response(response,
			GAMMA_COMMAND_SET_COMM_INTERFACE)) < 0)
		return err;
	...
	return err;
}	/* end * Function name: gamma_set_comm_interface */


/*************************************************************************
 *+
 * Function name: gamma_send_command
...
int gamma_send_command(char *port, char *cmd) {
	...
	if ( !Simulate ) {
		...
		if ( (err=socketport_write(socket_fd, cmd, strlen(cmd))) < 0) {
			if ((cerr = socketport_close(socket_fd)) < 0) {
				err = GAMMA_ERROR_CLOSE_PORT;
				errlog(SERROR, "%s %d: %s", __FILE__, __LINE__, 
					GammaErrMsg[GAMMA_ERROR_CODE0-err]);
			}
			pthread_mutex_unlock(&socket_mutex);
			return GAMMA_ERROR_WRITE_COMMAND;
		}
		log_msg(SINFO, LOGLVL_USER4,
	...
	return err;
}	/* end * Function name: gamma_send_command */


/*************************************************************************
 *+
 * Function name: gamma_send_request
...
int gamma_send_request(char *port, char *cmd, char *response) {
	...
	if ( !Simulate ) {
		...
		/* wait for response */
		usleep(GAMMA_TIME_BETWEEN_COMMANDS);
		...
		if ( (charsread=socketport_read(socket_fd, MAX_RESPONSE_LENGTH, 
			...
			pthread_mutex_unlock(&socket_mutex);
			return GAMMA_ERROR_READ_COMMAND;
		}
		log_msg(SINFO, LOGLVL_USER4,
	...
	return err;
}	/* end * Function name: gamma_send_request */


/*************************************************************************
 *+
 * Function name: gamma_create_command_string
 ...
 * Outputs: Returns length of command or GAMMA error code if there is an error.
 ...
 *-
*************************************************************************/
int gamma_create_command_string(
   char *outstring,	/* string to receive command	*/
   int bus_address,	/* bus address (1 = RS232)	*/
   ...
   ) 
{
   ...
   *  the GAMMA vacuum controller.
   *  See GAMMA vacuum SPCe controller user manual
   ...
   */
	...
	log_msg(SINFO, LOGLVL_USER3, "%s %d: entering %s", 
			__FILE__, __LINE__, __FUNCTION__);
  	log_msg(SDEBUG, LOGLVL_USER4, 
			"%s %d: Creating command string for GAMMA pump.", 
			__FILE__, __LINE__);
	...
	if (err == 0) {
		switch (command_code) {
			...
			case GAMMA_COMMAND_READ_MODEL:
			case GAMMA_COMMAND_READ_VERSION:
			case GAMMA_COMMAND_RESET:
			case GAMMA_COMMAND_GET_ARC_DETECT:
			case GAMMA_COMMAND_READ_CURRENT:
			case GAMMA_COMMAND_READ_PRESSURE:
			case GAMMA_COMMAND_READ_VOLTAGE:
			case GAMMA_COMMAND_GET_SUPPLY_STATUS:
			case GAMMA_COMMAND_GET_PUMP_SIZE:
			case GAMMA_COMMAND_GET_CAL_FACTOR:
			case GAMMA_COMMAND_GET_AUTO_RESTART:
			case GAMMA_COMMAND_START_PUMP:
			case GAMMA_COMMAND_STOP_PUMP:
			case GAMMA_COMMAND_GET_SETPOINT:
			case GAMMA_COMMAND_LOCK_KEYPAD:
			case GAMMA_COMMAND_UNLOCK_KEYPAD:
			case GAMMA_COMMAND_GET_ANALOG_MODE:
			case GAMMA_COMMAND_IS_HIGH_VOLTAGE_ON:
			case GAMMA_COMMAND_GET_HV_AUTORECOVERY:
			case GAMMA_COMMAND_SET_FIRMWARE_UPDATE:
			case GAMMA_COMMAND_GET_COMM_MODE:
			case GAMMA_COMMAND_GET_ETHERNET_MAC:
			case GAMMA_COMMAND_INITIATE_FEA:
			case GAMMA_COMMAND_INITIATE_HIPOT:
			...
			case GAMMA_COMMAND_GETSET_SERIAL_COMM:
			case GAMMA_COMMAND_GETSET_ETHERNET_IP:
			case GAMMA_COMMAND_GETSET_ETHERNET_MASK:
			case GAMMA_COMMAND_GETSET_ETHERNET_GTWY:
			case GAMMA_COMMAND_GETSET_HIPOT_TARGET:
			case GAMMA_COMMAND_GETSET_FOLDBACK_VOLTS:
			case GAMMA_COMMAND_GETSET_FOLDBACK_PRES:
				/* need trailing space for checksum */
			...
			/* command plus data */
			case GAMMA_COMMAND_SET_ARC_DETECT:
			case GAMMA_COMMAND_SET_PRESS_UNITS:
			case GAMMA_COMMAND_SET_PUMP_SIZE:
			case GAMMA_COMMAND_SET_CAL_FACTOR:
			case GAMMA_COMMAND_SET_AUTO_RESTART:
			case GAMMA_COMMAND_SET_SETPOINT:
			case GAMMA_COMMAND_SET_ANALOG_MODE:
			case GAMMA_COMMAND_SET_SERIAL_ADDRESS:
			case GAMMA_COMMAND_SET_HV_AUTORECOVERY:
			case GAMMA_COMMAND_SET_COMM_MODE:
			case GAMMA_COMMAND_SET_COMM_INTERFACE:
			case GAMMA_COMMAND_GET_FEA_DATA:
				/* need trailing space for checksum */
				sprintf(temp_command, " %.2X %s %s ", 
			...
			/* invalid command */
			default:
				err = GAMMA_ERROR_BAD_COMMAND_CODE;
	...
	return err;
}	/* end * Function name: gamma_create_command_string */


/*************************************************************************
 *+
 * Function name: gamma_validate_response
...
int gamma_validate_response(char *response, int command_code) {
	...
	if (bus != bus_address) {
		...
		/* bad response string */
		err = GAMMA_ERROR_INVALID_RESPONSE;
	...
	/* bus address OK */
		...
		if ( strncmp(substr,"ER",2) == 0 ) {
			...
			err = GAMMA_ERROR_CODE0 - atoi(substr);
		...
		/* status OK */
			...
			if (rcksm != cksm)
				err = GAMMA_ERROR_BAD_RESPONSE_CHECKSUM;
...
}	/* end * Function name: gamma_validate_response */

/*************************************************************************
 *+
 * Function name: getFloatFromGammaResponse

 * Description: Convert response to floating-point number.

 * Inputs: 
     char *gamma_response   -- string read from serialport
...
float getFloatFromGammaResponse(char *response) {
	...
	return num;
}	/* end * Function name: getFloatFromGammaResponse */


/*************************************************************************
 *+
 * Function name: getStringFromGammaResponse
...
int getStringFromGammaResponse(char *response, char *outstring) {
	...
	return err;
}	/* end * Function name: getStringFromGammaResponse */


/*************************************************************************
 *+
 * Function name: getIntFromGammaResponse
...
float getIntFromGammaResponse(char *response) {
	...
	return num;
}	/* end * Function name: getIntFromGammaResponse */
